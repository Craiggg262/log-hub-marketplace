...
DECLARE
  ...
  v_sms_count bigint;
BEGIN
  -- Allow admins OR unauthenticated calls (admin panel uses direct access)
  SELECT EXISTS (SELECT 1 FROM public.user_roles WHERE user_id = auth.uid() AND role = 'admin') OR auth.uid() IS NULL
    INTO v_is_admin;
